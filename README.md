...
Плановый сбор выполняется по будням в 18:20, 18:35 и 18:45 `Europe/Moscow`. Защита от повторов действует по тикеру и торговой дате: один и тот же сигнал не отправляется повторно, но последующие запуски могут прислать новые тикеры.
...
Миграция `0015_reliable_volume_notifications` добавляет счётчики аномалий IMOEX, отправленных и подавленных уведомлений для диагностики решения по широкому рынку.
