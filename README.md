...
- отдельная таблица контроля оборотов всех акций IMOEX;
- история последних 60 торговых сессий по каждому тикеру;
- сигнал при обороте от 3,6× до 6,5× относительно предыдущих 60 сессий;
- ежедневный сбор оборотов и одно сводное email-уведомление;
- настройка адреса получателя непосредственно в интерфейсе;
...
- `volume-worker` — планировщик и сборщик оборотов IMOEX из MOEX ISS;
- `db` — PostgreSQL 16;
...
- `GET /api/volume/overview` — последние показатели акций IMOEX;
- `GET /api/volume/securities/{ticker}/observations` — история тикера;
- `GET /api/volume/runs/latest` — состояние последнего сбора;
- `GET/PUT /api/volume/settings` — email получателя, только local;
...
### Таблица объёмов IMOEX

Кнопка `Объёмы IMOEX` открывает сводную таблицу без перехода в отдельное приложение. Для каждой бумаги показываются вес в индексе, дата, оборот, средний оборот, коэффициент и статус. По тикеру открывается история последних 60 сессий.
...
- `Обычно` — ниже 3,6×;
- `Мало истории` — меньше 60 завершённых сессий.
...
Адрес получателя в `.env` не хранится: его вводят на странице `/volumes/` из локальной сети. Для отключения уведомлений достаточно очистить это поле или установить `VOLUME_SMTP_ENABLED=false`.

Не помещайте реальный пароль в `.env.example`, Git или Kubernetes manifest. Для Minikube SMTP-логин и ключ задаются в Secret `moex-secrets`, а остальные параметры — в `k8s/backend.yaml` и `k8s/volume-worker.yaml`.
...
Тесты поднимают статический frontend на `127.0.0.1:4173`, подменяют API-ответы и проверяют обе страницы: 17-колоночную таблицу прогнозов и таблицу/детализацию объёмов IMOEX. HTML-отчёт создаётся в `playwright-report/`.
