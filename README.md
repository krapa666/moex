...
Таблица сгруппирована по двум календарным прогнозным годам. На широком экране она ограничена компактной шириной и центрируется, а на более узком пропорционально сжимает все 15 столбцов без горизонтальной прокрутки. Заголовки столбцов выровнены по центру, числовые поля — вправо.
...
Тесты поднимают статический frontend на `127.0.0.1:4173`, подменяют API-ответы и проверяют двухлетнюю таблицу в Chromium на ширинах от 1024 до 2560 пикселей. HTML-отчёт создаётся в `playwright-report/`.
