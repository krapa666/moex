...
test('shows same-year price gap, analyst targets and return metrics', async ({ page }) => {
  ...
  await expect(panel.locator('[data-consensus-target-return="1"]')).toHaveClass(/positive/);
  ...
  await expect(panel.locator('[data-consensus-target-return="2"]')).toHaveClass(/negative/);
...
test('keeps market gap useful with a single comparable target without inferring agreement', async ({ page }) => {
  ...
  await expect(panel.locator('.analytics-consensus-notes')).toContainText('минимум две сопоставимые цели');
});
