...
Страница `/analytics/` объединяет текущий consensus, историю прогнозных ревизий, динамику consensus и оценку исторической точности источников.
...
**Рейтинг не меняет текущий consensus автоматически.** До завершения отдельного backtest веса остаются равными в том смысле, что текущая UI-агрегация продолжает использовать обычную медиану сопоставимых целей.
...
- accuracy-weighted consensus пока намеренно отключён.
